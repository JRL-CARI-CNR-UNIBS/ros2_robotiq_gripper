...
RobotiqSocket::Status RobotiqSocket::connect(const double socket_timeout) {
    ...
    boost::asio::async_connect(*socket_, endpoints, [&](const boost::system::error_code& ec, const tcp::endpoint&) {
        if (!ec) {
            connected = true;
            timer.cancel();
        } else {
...
bool RobotiqSocket::auto_calibration(bool log) {
    ...
    if (res.object_status != ObjectStatus::AT_DEST) {
        std::cout << "Calibration failed opening to start: " << static_cast<int>(res.object_status) << std::endl;
        return false;
    }
    ...
    if (res.object_status != ObjectStatus::AT_DEST) {
        std::cout << "Calibration failed because of an object: " << static_cast<int>(res.object_status) << std::endl;
        return false;
    }
    ...
    if (log) {
        std::cout << "Gripper auto-calibrated to [" << get_open_position() << ", " << get_closed_position() << "]" << std::endl;
    }
