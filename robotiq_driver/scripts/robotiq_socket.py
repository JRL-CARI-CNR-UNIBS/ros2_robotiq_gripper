...
class RobotiqCModelURCap:
    ...
    def disconnect(self):
        ...
        if not self.socket:
            raise RuntimeError("Socket is not connected. Cannot disconnect.")
        self.socket.close()
    ...
    def _set_vars(self, var_dict):
        ...
        with self.command_lock:
            if not self.socket:
                raise RuntimeError("Socket is not connected. Cannot send command.")
    ...
    def get_current_effort(self):
        """Returns the current effort of the gripper."""
    ...
    def auto_calibrate(self, log = True):
        ...
        (position, status) = self.move_and_wait_for_pos(self.get_open_position(),
                                                        self.CALIBRATION_SPEED,
                                                        self.CALIBRATION_FORCE) # type: ignore
        ...
        (position, status) = self.move_and_wait_for_pos(self.get_closed_position(),
                                                        self.CALIBRATION_SPEED,
                                                        self.CALIBRATION_FORCE) # type: ignore
        if RobotiqCModelURCap.ObjectStatus(status) != RobotiqCModelURCap.ObjectStatus.AT_DEST:
    ...
    def move_and_wait_for_pos(self, position, speed, force):
        ...
        while RobotiqCModelURCap.ObjectStatus(cur_obj) == RobotiqCModelURCap.ObjectStatus.MOVING:
            cur_obj = self._get_var(self.OBJ)
        # report the actual position and the object status
        final_pos = self._get_var(self.POS)
        if (RobotiqCModelURCap.ObjectStatus(cur_obj) == RobotiqCModelURCap.ObjectStatus.STOPPED_INNER_OBJECT) or \
            (RobotiqCModelURCap.ObjectStatus(cur_obj) == RobotiqCModelURCap.ObjectStatus.STOPPED_OUTER_OBJECT):
            self._set_var(self.GTO, 0)
        final_obj = cur_obj
        return final_pos, RobotiqCModelURCap.ObjectStatus(final_obj)
    
    def is_moving(self):
        """Returns whether the gripper is currently moving."""
        return RobotiqCModelURCap.ObjectStatus(self._get_var(self.OBJ)) == RobotiqCModelURCap.ObjectStatus.MOVING

    

### Example stand-alone test code
# if __name__ == '__main__':
#     gripper = RobotiqCModelURCap()
